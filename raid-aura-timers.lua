...
local module = ShaguTweaks:register({
  title = T["Aura Timers"],
  description = T["Show buff and debuff icons with duration timers below target, party, pet, and raid frames. Requires SuperWoW."],
...
local TIMER_HEIGHT = 10
local RAID_TIMER_FONT_SIZE = 7
local RAID_TIMER_HEIGHT = 8
local MAX_BUFFS = 16
...
local function CreateAuraIcon(parent, iconSize, isDebuff, timerFontSize)
  timerFontSize = timerFontSize or TIMER_FONT_SIZE
...
local function CreateAuraDisplay(parent, maxBuffs, maxDebuffs, iconSize, xOffset, yOffset, timerFontSize, growDir)
  timerFontSize = timerFontSize or TIMER_FONT_SIZE
  ...
  if growDir == "RIGHT" then
    -- Debuffs first (leftmost), then buffs continue to the right
    for i = 1, maxDebuffs do
      local icon = CreateAuraIcon(parent, iconSize, true, timerFontSize)
      icon:SetPoint("TOPLEFT", parent, "BOTTOMLEFT", xOffset + (i - 1) * step, yOffset)
      display.debuffs[i] = icon
    end
    for i = 1, maxBuffs do
      local icon = CreateAuraIcon(parent, iconSize, false, timerFontSize)
      icon:SetPoint("TOPLEFT", parent, "BOTTOMLEFT", xOffset + (maxDebuffs + i - 1) * step, yOffset)
      display.buffs[i] = icon
    end
  else
    -- "LEFT": Debuffs first (rightmost edge), then buffs continue to the left
    for i = 1, maxDebuffs do
      local icon = CreateAuraIcon(parent, iconSize, true, timerFontSize)
      icon:SetPoint("TOPRIGHT", parent, "BOTTOMRIGHT", -(xOffset + (i - 1) * step), yOffset)
      display.debuffs[i] = icon
    end
    for i = 1, maxBuffs do
      local icon = CreateAuraIcon(parent, iconSize, false, timerFontSize)
      icon:SetPoint("TOPRIGHT", parent, "BOTTOMRIGHT", -(xOffset + (maxDebuffs + i - 1) * step), yOffset)
      display.buffs[i] = icon
    end
  end
...
local function UpdateDisplay(display, maxBuffs, maxDebuffs, unitstr)
  ...
  -- Update debuffs first, count how many are visible
  UpdateBuffIcons(display.debuffs, maxDebuffs, unitstr, guid, texToSpell, true)
  local visibleDebuffs = 0
  for i = 1, maxDebuffs do
    if display.debuffs[i]:IsShown() then
      visibleDebuffs = i
    end
  end

  -- Reposition buff icons to sit right after visible debuffs
  local step = display.iconSize + ICON_SPACING
  for i = 1, maxBuffs do
    local icon = display.buffs[i]
    icon:ClearAllPoints()
    if display.growDir == "RIGHT" then
      icon:SetPoint("TOPLEFT", display.parent, "BOTTOMLEFT", display.xOffset + (visibleDebuffs + i - 1) * step, display.yOffset)
    else
      icon:SetPoint("TOPRIGHT", display.parent, "BOTTOMRIGHT", -(display.xOffset + (visibleDebuffs + i - 1) * step), display.yOffset)
    end
  end

  UpdateBuffIcons(display.buffs, maxBuffs, unitstr, guid, texToSpell, false)
end
...
module.enable = function(self)
      ...
      if display.buffs then
        for _, icon in ipairs(display.buffs) do
          if icon:IsShown() and icon.timerDuration and icon.timerStart then
            ...
            if remaining > 0 then
              icon.timer:SetText(TimeConvert(remaining))
              icon.timer:Show()
            else
              icon.timer:SetText("")
              icon.timer:Hide()
  ...
  do
    local targetDisplay = CreateAuraDisplay(TargetFrame, MAX_BUFFS, MAX_DEBUFFS,
      TARGET_ICON_SIZE, 5, 30, nil, "RIGHT")
    ...
    for i = 1, 4 do
      ...
      if frame then
        partyDisplays[i] = CreateAuraDisplay(frame, MAX_BUFFS, MAX_DEBUFFS,
          PARTY_ICON_SIZE, 12, 20, nil, "LEFT")
  ...
  if PetFrame then
    local petDisplay = CreateAuraDisplay(PetFrame, MAX_BUFFS, MAX_DEBUFFS,
      PARTY_ICON_SIZE, 14, 11, nil, "LEFT")
  ...
  -- ===========================================================================
  -- RAID FRAMES (ShaguTweaks component)
  -- ===========================================================================
  if ShaguTweaks.UnitFrame_NewComponent then
    ShaguTweaks.UnitFrame_NewComponent('aura-timers', {
      events = {
        'UNIT_AURA',
      },
      ...
      create = function(frame)
        ...
        for i = 1, RAID_MAX_BUFFS do
          local icon = CreateAuraIcon(frame, RAID_ICON_SIZE, false, RAID_TIMER_FONT_SIZE)
          icon:SetPoint("BOTTOMLEFT", frame, "TOPLEFT", (i - 1) * (RAID_ICON_SIZE + 1), 1)
          frame.auraBuffs[i] = icon
        end

        for i = 1, RAID_MAX_DEBUFFS do
          local icon = CreateAuraIcon(frame, RAID_ICON_SIZE, true, RAID_TIMER_FONT_SIZE)
          icon:SetPoint("TOPLEFT", frame, "BOTTOMLEFT", (i - 1) * (RAID_ICON_SIZE + 1), -1)
          frame.auraDebuffs[i] = icon
        end

        local raidDisplay = { buffs = frame.auraBuffs, debuffs = frame.auraDebuffs }
        table.insert(allDisplays, raidDisplay)
      end,

      update = function(frame, event)
        if not event then return end
        if arg1 and frame.unitstr and frame.unitstr ~= arg1 then return end

        local unitstr = frame.unitstr
        if not unitstr or not UnitExists(unitstr) then
          for i = 1, RAID_MAX_BUFFS do frame.auraBuffs[i]:Hide() end
          for i = 1, RAID_MAX_DEBUFFS do frame.auraDebuffs[i]:Hide() end
          return
        end
        ...
        UpdateBuffIcons(frame.auraBuffs, RAID_MAX_BUFFS, unitstr, guid, texToSpell, false)
        UpdateBuffIcons(frame.auraDebuffs, RAID_MAX_DEBUFFS, unitstr, guid, texToSpell, true)
      end,
    })
  end
end
